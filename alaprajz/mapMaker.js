...
canvas.onclick = function (e) {
	...
	if (squaringPhase || beaconPhase) {
		for (let i = 0; i < coords.length; ++i) {
			...
			} else if (coords[i].firstPoint.x === coords[i].secondPoint.x
				&& coords[i].firstPoint.y < coords[i].secondPoint.y) {
				if (e.layerX > coords[i].firstPoint.x - 5 && e.layerX < coords[i].secondPoint.x + 5 &&
					e.layerY > coords[i].firstPoint.y && e.layerY < coords[i].secondPoint.y) {
					if (squaringPhase) {
			...
			} else if (coords[i].firstPoint.y === coords[i].secondPoint.y
	...
	if (routePhase) {
		...
		const point = {x: e.layerX, y: e.layerY};
		let p;
		for (let i = 0; i < graph.peaks.length; ++i) {
			if (point.x - 10 < graph.peaks[i].x && point.x + 10 > graph.peaks[i].x &&
				point.y - 10 < graph.peaks[i].y && point.y + 10 > graph.peaks[i].y) {
					b = true;
					p = graph.peaks[i];
				}
		}
		...
		if (b) {
			if (selected !== undefined) {
				for (let i = 0; i < graph.edges.length; ++i) {
					if (graph.edges[i].from.id === selected.id && graph.edges[i].to.id === p.id
						|| graph.edges[i].to.id === selected.id && graph.edges[i].from.id === p.id) {
							return;
						}
				}
				connectPeaks(selected, p);
				selected = undefined;
				draw();
			} else {
				selected = p;
...
function next() {
	if (drawingPhase) {
		...
		console.log("Jeladók felhelyezése");
		let b = false;
...
const draw = function (e) {
	...
	if (routePhase) {
		...
		for (let i = 0; i < graph.peaks.length; ++i) {
			...
			context.beginPath();
			context.arc(graph.peaks[i].x, graph.peaks[i].y, 5, 0, 2 * Math.PI);
			context.stroke();
			if (selected !== undefined && graph.peaks[i].id === selected.id) {
				context.fill();
			}
		}
		...
		for (let i = 0; i < graph.edges.length; ++i) {
			context.beginPath();
			context.moveTo(graph.edges[i].from.x, graph.edges[i].from.y);
			context.lineTo(graph.edges[i].to.x, graph.edges[i].to.y);
			context.stroke();
		}
		/* context.beginPath();
		context.moveTo(prevX, prevY);
